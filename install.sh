...
step_breakglass() {
  ...
  if grep -q "^BREAKGLASS_PASSWORD_HASH=" "${INSTALL_DIR}/.env" 2>/dev/null; then
    ok "Break-glass credential already configured"
    return
  fi
  ...
  bg_hash=$(cd "$INSTALL_DIR" && node -e "const b=require('bcryptjs');console.log(b.hashSync('$bg_pw',10))")

  # Write static block with single-quoted heredoc (no variable expansion in content)
  cat >> "${INSTALL_DIR}/.env" <<'BGENV'

# Break-glass emergency admin account — last-resort access when all admins are locked out.
# Reset: generate a new hash with node -e "require('bcryptjs').hashSync('<pw>',10)" and restart.
...
step_fail2ban() {
  if ! prompt_yesno "fail2ban" \
    "Install fail2ban to block SSH brute-force attacks?

Bans an IP for 1 hour after 5 failed SSH login attempts.
Recommended for internet-facing servers."; then
  ...
  cat > /etc/fail2ban/jail.d/learn.conf <<'JAIL'
[sshd]
enabled  = true
maxretry = 5
findtime = 600
...
step_firewall() {
  if ! prompt_yesno "nftables Firewall" \
    "Set up a basic nftables firewall?

Allows: SSH (22), HTTP (80), HTTPS (443), App (${APP_PORT})
Drops all other inbound traffic.

Recommended for internet-facing servers."; then
    note "Firewall setup skipped."
    return
  fi

  msg "Installing nftables..."
  apt-get install -y -qq nftables
  ok "nftables installed"

  msg "Writing firewall rules..."
  cat > /etc/nftables.conf <<NFT
#!/usr/sbin/nft -f
# Learn Platform firewall \u2014 generated by install.sh
flush ruleset
...
step_ssh_harden() {
  ...
  note "\u2502                                                            \u2502"
  note "\u2502 Paste your PUBLIC key (from ~/.ssh/id_*.pub) or leave      \u2502"
  note "\u2502 blank to skip.                                             \u2502"
  note "\u2514\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2518"
  echo ""
  ...
  if [[ -n "$ssh_pubkey" ]]; then
    ...
    note "To SSH in: ssh root@<server-ip>  (using your private key)"
  else
    note "SSH key not provided \u2014 password auth remains enabled."
    note "To harden later:"
    note "  echo '<pub-key>' >> /root/.ssh/authorized_keys"
    note "  sed -i 's/^#*PasswordAuthentication.*/PasswordAuthentication no/' /etc/ssh/sshd_config"
    note "  systemctl restart sshd"
  fi
}
