...
interface VocabularyBuilderProps {
  ...
  vocabItems: Array<{ word: string; translation: string; image?: string }>;
  setVocabItems: React.Dispatch<
    React.SetStateAction<Array<{ word: string; translation: string; image?: string }>>
  >;
...
export function VocabularyBuilder({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Picture quiz builder card when enabled */}
      {pictureSupplementation && vocabItems.length > 0 && (
        <div className="p-6 border rounded border-neutral-300 dark:border-neutral-850 bg-white dark:bg-neutral-900 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-2">
            <div>
              <h4 className="text-xs font-extrabold uppercase font-mono tracking-wider text-purple-650 dark:text-purple-400 flex items-center gap-1.5">
                ...
                Picture Supplementation Settings
              </h4>
              <p className="text-[11px] text-neutral-450 mt-0.5">
                Associate images with each word. These will be used for distractor selection in the Picture Match stage.
              </p>
            </div>
          ...
          {/* List of vocabulary words image configurations */}
          <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
            {vocabItems.map((item, idx) => (
              <div
                key={idx}
                className="p-3 border rounded-lg bg-neutral-50/50 dark:bg-neutral-950/15 flex flex-col sm:flex-row sm:items-center justify-between gap-3"
              >
                ...
                {uploadStatusIdx === idx && (
                  <span className="text-[9px] font-mono text-purple-650 dark:text-purple-400 block w-full mt-1">
                    {uploadStatusText}
                  </span>
                )}
              </div>
